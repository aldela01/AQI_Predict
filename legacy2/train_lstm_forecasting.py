"""
Direct multi-output LSTM forecasting pipeline using the AirQualityPreprocessor and
TensorFlow's WindowGenerator pattern.

This script:
- Builds time windows with tf.keras.utils.timeseries_dataset_from_array.
- Trains a multi-step LSTM that predicts all future steps in one pass.
...
class WindowGenerator:
    ...
    def make_dataset(self, data: np.ndarray, shuffle: bool) -> tf.data.Dataset:
        ...
        if self.time_feature_indices:
            ...
            ds = ds.map(
                lambda inputs, labels, mask, future: (inputs, labels),
                num_parallel_calls=tf.data.AUTOTUNE,
            )
        if shuffle:
            ds = ds.shuffle(
...
def get_pm25_columns(df: pd.DataFrame) -> list[str]:
    return [col for col in df.columns if col.endswith("PM2.5")]


def compute_winsor_limits(
    train_df: pd.DataFrame, columns: list[str], lower_q: float, upper_q: float
) -> dict[str, tuple[float, float]]:
    limits: dict[str, tuple[float, float]] = {}
    for col in columns:
        series = pd.to_numeric(train_df[col], errors="coerce").replace(-9999, np.nan)
...
def apply_winsor_limits(
    df: pd.DataFrame, limits: dict[str, tuple[float, float]]
) -> pd.DataFrame:
    df = df.copy()
    for col, (lower, upper) in limits.items():
        if col not in df.columns:
            continue
        series = pd.to_numeric(df[col], errors="coerce").replace(-9999, np.nan)
...
def build_model(
    input_shape: Tuple[int, int], future_feature_count: int, cfg: Config
) -> tf.keras.Model:
    ...
    if cfg.num_lstm_layers < len(lstm_units):
        lstm_units = lstm_units[: cfg.num_lstm_layers]
    if cfg.num_lstm_layers > len(lstm_units):
        lstm_units.extend([lstm_units[-1]] * (cfg.num_lstm_layers - len(lstm_units)))

    reg = tf.keras.regularizers.l2(cfg.l2_reg) if cfg.l2_reg > 0 else None

    for i, units in enumerate(lstm_units):
        return_sequences = i < (cfg.num_lstm_layers - 1)
    ...
    for units in dense_units:
        x = tf.keras.layers.Dense(units, activation=cfg.dense_activation)(x)
        if cfg.dense_dropout > 0:
            x = tf.keras.layers.Dropout(cfg.dense_dropout)(x)

    future_inputs = None
    if future_feature_count > 0:
        future_inputs = tf.keras.Input(
            shape=(cfg.out_steps, future_feature_count), name="future_covariates"
        )
        future_flat = tf.keras.layers.Flatten()(future_inputs)
        x = tf.keras.layers.Concatenate()([x, future_flat])

    outputs = tf.keras.layers.Dense(
        cfg.out_steps, activation=cfg.output_activation, dtype="float32"
    )(x)
    outputs = tf.keras.layers.Reshape([cfg.out_steps, 1])(outputs)

    if future_inputs is not None:
        return tf.keras.Model([inputs, future_inputs], outputs)
...
def main() -> None:
    ...
    winsor_limits = None
    if cfg.winsorize_pm25:
        if not (0.0 <= cfg.winsor_lower_q < cfg.winsor_upper_q <= 1.0):
            raise ValueError("winsor_lower_q must be < winsor_upper_q within [0, 1].")
        pm25_cols = get_pm25_columns(splits["train"])
        if pm25_cols:
            winsor_limits = compute_winsor_limits(
                splits["train"],
                pm25_cols,
                cfg.winsor_lower_q,
                cfg.winsor_upper_q,
            ...
            for split_name, split_df in splits.items():
                splits[split_name] = apply_winsor_limits(split_df, winsor_limits)
    ...
    model = build_model(input_shape, len(time_feature_columns), cfg)
    ...
    if winsor_limits is not None:
        results["winsor_limits"] = winsor_limits

...
class SeasonalNaiveConfig:
    ...
    run_name: str = "baseline_seasonal_naive_24h"
...
def run(cfg: SeasonalNaiveConfig) -> None:
    ...
    # Necesitamos pm25 observado (para construir el naive)
    df = df[df["pm25"].notna() & df["timestamp"].notna()].copy()
    df = df.sort_values(["station_id", "timestamp"])

    y_cols = [f"y_h{h:02d}" for h in range(1, cfg.horizon + 1)]

    # Precompute predicciones por horizonte como shifts de pm25:
    # pred_h = pm25(t - (season - h)) = shift(season - h)
    pred_cols = []
    for h in range(1, cfg.horizon + 1):
        lag = cfg.season - h  # 23..0
        col = f"pred_h{h:02d}"
        df[col] = df.groupby("station_id")["pm25"].shift(lag)
    ...
    with mlflow.start_run(run_name=cfg.run_name):
        ...
        for split in ["test", "val"]:
            ...
            y_pred = part[pred_cols].to_numpy()

            m, s = compute_metrics(y_true, y_pred)
            m["split"] = split
            m["baseline"] = "seasonal_naive_24h"
            outputs.append(m)

            summaries.append({"baseline": "seasonal_naive_24h", "split": split, **s})
            ...
            # Series por horizonte (útil en MLflow)
        ...
        out_by_h = cfg.reports_dir / "seasonal_naive_h24_metrics_by_horizon.csv"
        metrics.to_csv(out_by_h, index=False)

        summary = pd.DataFrame(summaries)
        out_sum = cfg.reports_dir / "seasonal_naive_h24_metrics_summary.csv"

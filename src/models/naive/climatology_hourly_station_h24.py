...
class ClimatologyConfig:
    ...
    run_name: str = "baseline_climatology_station_hour"
...
def run(cfg: ClimatologyConfig) -> None:
    ...
    # Necesitamos hour y pm25 (para construir la climatología)
    df = df[df["timestamp"].notna() & df["hour"].notna()].copy()
    df["hour"] = df["hour"].astype(int)

    # Entrenar climatología SOLO con train (pm25 observado)
    train = df[(df["split"] == "train") & df["pm25"].notna()].copy()
    if len(train) == 0:
        raise ValueError("No hay datos pm25 en train para construir climatología.")

    mu_station_hour = train.groupby(["station_id", "hour"])["pm25"].mean()
    mu_hour = train.groupby("hour")["pm25"].mean()
    mu_global = float(train["pm25"].mean())
    ...
    with mlflow.start_run(run_name=cfg.run_name):
        mlflow.log_params(
            {
                ...
                "baseline_type": "climatology_station_hour",
                "horizon": cfg.horizon,
        ...
        for split in ["test", "val"]:
            ...
            # y_pred: matriz (n, horizon)
            n = len(part)
            y_pred = np.full((n, cfg.horizon), np.nan, dtype=float)

            st = part["station_id"].to_numpy()
            hour_now = part["hour"].to_numpy(dtype=int)

            for i in range(cfg.horizon):
                h = i + 1
                hour_f = (hour_now + h) % 24

                idx = pd.MultiIndex.from_arrays(
                    [st, hour_f], names=["station_id", "hour"]
                )
                pred = mu_station_hour.reindex(idx).to_numpy()

                # fallback a climatología global por hora
                pred2 = mu_hour.reindex(hour_f).to_numpy()
                pred = np.where(np.isfinite(pred), pred, pred2)

                # fallback final a media global
                pred = np.where(np.isfinite(pred), pred, mu_global)

                y_pred[:, i] = pred

            m, s = compute_metrics(y_true, y_pred)
            m["split"] = split
            m["baseline"] = "climatology_hourly_station"
            outputs.append(m)

            summaries.append(
                {"baseline": "climatology_hourly_station", "split": split, **s}
            )
            ...
            mlflow.log_metrics(
                {
                    ...
                    f"n_eval_{split}_mean": s["n_eval_mean"],
                    f"n_eval_{split}_h24": s["n_eval_h24"],
                }
            )

            for _, row in m.iterrows():
                hh = int(row["horizon_h"])
                mlflow.log_metric(f"rmse_{split}", float(row["rmse"]), step=hh)
                mlflow.log_metric(f"r2_{split}", float(row["r2"]), step=hh)

        metrics = pd.concat(outputs, ignore_index=True)
        out_by_h = (
            cfg.reports_dir / "climatology_hourly_station_h24_metrics_by_horizon.csv"
        )
        metrics.to_csv(out_by_h, index=False)

        summary = pd.DataFrame(summaries)
        out_sum = cfg.reports_dir / "climatology_hourly_station_h24_metrics_summary.csv"
